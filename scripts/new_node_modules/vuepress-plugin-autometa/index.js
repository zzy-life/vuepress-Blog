...
PLUGIN.get_default_description = ( $page, options ) =>
{
	...
	for ( const source of options['description_sources'] )
	{
        ...
        if (content) {
            ...
            // 移除Markdown格式，包括链接、图片、强调、标题等
            cleanedText = cleanedText.replace(/!\[.*?\]\(.*?\)/g, '')  // 移除图片
                                    .replace(/\[.*?\]\(.*?\)/g, '')   // 移除链接
                                    .replace(/#+\s/g, '')            // 移除标题
                                    .replace(/[*_]{1,2}/g, '')       // 移除斜体和粗体
                                    .replace(/~~/g, '')              // 移除删除线
                                    .replace(/`{1,3}/g, '')          // 移除代码格式
                                    .replace(/[>\+\-\[\](){}]+/g, ''); // 移除其它Markdown特殊字符
        
            // 替换掉连续的空白字符为一个空格
            cleanedText = cleanedText.replace(/\s\s+/g, ' ');
        
            // 提取前 120 个字，尽量按照单词边界
            let summary = cleanedText.split(/\s+/).reduce((acc, word) => {
                if (acc.length + word.length <= 120) {
                    return acc + (acc.length > 0 ? ' ' : '') + word;
                } else {
                    return acc;
                }
            }, '');
            ...
            // 如果原始文本被截断，添加省略号
            if (cleanedText.length > summary.length) {
                summary += '...';
            }
